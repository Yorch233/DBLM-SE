...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, dim: int = 16, stride: int = 1, dilations=(1, 3, 9)):
        ...
        self.block = nn.Sequential(
            *runits,
            Activation1d(
                activation=activations.SnakeBeta(dim //
                                                 2, alpha_logscale=True)),
            WNConv1d(
                dim // 2,
                dim,
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.block.extend(
            [ResidualUnit(output_dim, dilation=d) for d in dilations])
...
class ConvNeXtBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.dwconv = nn.Conv1d(dim, dim, kernel_size=7, padding=3,
    ...
    def forward(
            self,
            x: torch.Tensor,
            cond_embedding_id: Optional[torch.Tensor] = None) -> torch.Tensor:
        residual = x
        x = self.dwconv(x)
...
class AdaLayerNorm(nn.Module):
    ...
    def __init__(self,
        ...
        self.scale = nn.Embedding(num_embeddings=num_embeddings,
                                  embedding_dim=embedding_dim)
        self.shift = nn.Embedding(num_embeddings=num_embeddings,
                                  embedding_dim=embedding_dim)
        torch.nn.init.ones_(self.scale.weight)
        torch.nn.init.zeros_(self.shift.weight)

    def forward(self, x: torch.Tensor,
                cond_embedding_id: torch.Tensor) -> torch.Tensor:
...
class ResBlock1(nn.Module):
    ...
    def __init__(
        ...
        self.gamma = nn.ParameterList([
            nn.Parameter(layer_scale_init_value * torch.ones(dim, 1),
                         requires_grad=True)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        for c1, c2, gamma in zip(self.convs1, self.convs2, self.gamma):
            xt = torch.nn.functional.leaky_relu(
                x, negative_slope=self.lrelu_slope)
            xt = c1(xt)
            xt = torch.nn.functional.leaky_relu(
                xt, negative_slope=self.lrelu_slope)
            xt = c2(xt)
...
class SemanticEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.final_conv = nn.Conv1d(in_channels=encode_channels,
...
class SemanticDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.initial_conv = nn.Conv1d(in_channels=code_dim,
        ...
        self.final_conv = nn.Conv1d(in_channels=decode_channels,
