...
def pad_audio_batch(batch):
    ...
    return torch.tensor(padded_audios), torch.tensor(
        padded_feat_list), fname_list, audio_length
...
class WaveDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        feat = self.feature_extractor(
            audio_pad, sampling_rate=self.sampling_rate,
...
def save_vq_code(vq_codes: torch.Tensor, wav_paths: List[str],
                 lengths: List[int], output_dir: str):
    for i, wav_path in enumerate(wav_paths):
        relative_path = os.path.relpath(wav_path, args.input_dir)
        code_path = os.path.join(output_dir, 'vq_codes',
                                 relative_path.replace('.flac', '.npy'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--flist_file',
                        type=str,
                        default='/path/to/file.txt',
                        help='TSV file containing paths to audio files')
    parser.add_argument('--ckpt',
                        type=str,
                        default='/path/to/epoch=4-step=1400000.ckpt',
                        help='Path to the model checkpoint')
    parser.add_argument('--output-dir',
                        type=str,
                        default='/path/to/saving_code_folder',
                        help='Output directory for saving audio files')
    ...
    SemanticEncoder_module.load_state_dict(
    ...
    df = pd.read_csv(args.flist_file,
                     sep='\t',
                     header=None,
                     names=['filename', 'duration'],
                     skiprows=1)
    ...
    dataset = WaveDataset(file_list=current_file_list,
                          sampling_rate=sr,
