...
class MultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        self.q_proj = quant_noise(nn.Linear(embed_dim, q_embed_dim, bias=bias),
                                  q_noise, qn_block_size)
    ...
    def forward(
        ...
        if (not is_tpu  # don't use PyTorch version on TPUs
                ...
                and not torch.jit.is_scripting() and self.q_head_dim
                == self.head_dim):
            ...
            if position_bias is not None:
                ...
                if self.gru_rel_pos:
                    ...
                    new_x_shape = query_layer.size()[:-1] + (self.num_heads,
                                                             -1)
                    ...
                    gate_a, gate_b = torch.sigmoid(
                        self.grep_linear(query_layer).view(
                            _B, _H, _L, 2, 4).sum(-1,
                                                  keepdim=False)).chunk(2,
                                                                        dim=-1)
        ...
        q = (q.contiguous().view(tgt_len, bsz * self.num_heads,
                                 self.q_head_dim).transpose(0, 1))
        if k is not None:
            k = (k.contiguous().view(-1, bsz * self.num_heads,
                                     self.k_head_dim).transpose(0, 1))
        if v is not None:
